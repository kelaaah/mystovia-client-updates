...
function init()
  for k,v in pairs(defaultOptions) do
    ...
    options[k] = v  -- Set defaults initially, setup() will override with saved values
  end
  ...
  -- hide topmenu buttons; they will be relocated to EnterGame and bottom-right corner
...
function terminate()
  ...
  -- Save all current options before terminating
  ...
  -- optionsButton and audioButton are now managed elsewhere
...
function setup()
  ...
  for k,v in pairs(defaultOptions) do
    ...
    if type(v) == 'boolean' then
      savedValue = g_settings.getBoolean(k)
    elseif type(v) == 'number' then
      savedValue = g_settings.getNumber(k)
    elseif type(v) == 'string' then
      savedValue = g_settings.getString(k)
    end

    -- Store the loaded value in options array
    options[k] = savedValue

    -- Apply to UI and game
    setOption(k, savedValue, true)
  end
  ...
  -- ensure audio mute state is applied on startup (mute by setting gain to 0)
  if g_sounds and not g_settings.getBoolean('enableAudio') then
    pcall(function()
      local musicChannel = g_sounds.getChannel(SoundChannels.Music)
      if musicChannel then musicChannel:setGain(0) end
      local botChannel = g_sounds.getChannel(SoundChannels.Bot)
      if botChannel then botChannel:setGain(0) end
    end)
  end
...
function setOption(key, value, force)
  ...
  elseif key == 'enableAudio' then
    if g_sounds ~= nil then
      -- use setGain(0) to mute instead of setEnabled(false) to avoid stopping music
      local musicChannel = g_sounds.getChannel(SoundChannels.Music)
      local botChannel = g_sounds.getChannel(SoundChannels.Bot)
      if value then
        ...
        if musicChannel then 
          ...
          local musicVol = g_settings.getNumber('musicSoundVolume') or 50
          musicChannel:setGain(musicVol/100)
        end
        if botChannel then 
          botChannel:setEnabled(true)
          local botVol = g_settings.getNumber('botSoundVolume') or 50
          botChannel:setGain(botVol/100)
        end
      else
        -- mute: set gain to 0 (silent but still playing)
        if musicChannel then musicChannel:setGain(0) end
        if botChannel then botChannel:setGain(0) end
      end
    end
  ...
  elseif key == 'musicSoundVolume' then
    ...
    if audioPanel then
      local label = audioPanel:getChildById('musicSoundVolumeLabel')
      if label then
  ...
  elseif key == 'botSoundVolume' then
    ...
    if audioPanel then
      local label = audioPanel:getChildById('botSoundVolumeLabel')
      if label then

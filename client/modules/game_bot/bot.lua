...
function refresh()
  ...
  for i=1,#configs do
    ...
    -- Skip cavebot_1.3 and vBot_4.7
    if name ~= "cavebot_1.3" and name ~= "vBot_4.7" then
      configList:addOption(name)
    end
  end
...
function createDefaultConfigs()
  -- Copy (and overwrite) default configs into /bot so runtime copy matches default_configs
  ...
  for i, config_name in ipairs(defaultConfigFiles) do
    ...
    local defaultFiles = g_resources.listDirectoryFiles("default_configs/" .. config_name, true, false)
    for _, file in ipairs(defaultFiles) do
      local parts = file:split("/")
      local name = parts[#parts]
      -- build destination path under /bot
      local relPath = file:sub(#("default_configs/" .. config_name) + 2) -- strip prefix
      local destPath = "/bot/" .. config_name .. "/" .. relPath

      -- ensure destination subdirs exist
      local dirParts = relPath:split("/")
      if #dirParts > 1 then
        local current = "/bot/" .. config_name
        for j = 1, #dirParts-1 do
          current = current .. "/" .. dirParts[j]
          if not g_resources.directoryExists(current) then
            g_resources.makeDir(current)
            if not g_resources.directoryExists(current) then
              return onError("Can't create " .. current .. " directory in " .. g_resources.getWriteDir())
            end
          end
        end
      end

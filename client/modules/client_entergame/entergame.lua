...
function EnterGame.init()
  ...
  -- do not add a generic 'Another' option; only configured servers should appear
  -- ensure Mystovia is selected by default when available
  if serverSelector:isOption("Mystovia") then
    serverSelector:setCurrentOption("Mystovia", false)
  end
...
function EnterGame.openCreateAccount()
  -- open the create account webpage in external browser
  scheduleEvent(function()
    if g_platform and g_platform.openUrl then
      pcall(function()
        g_platform.openUrl("https://otserver-monorepo.onrender.com/")
      end)
    end
  end, 50)
end

function EnterGame.exit()
  -- exit the client
  scheduleEvent(function()
    g_app.exit()
  end, 50)
...
function EnterGame.toggleOptions()
...
function EnterGame.toggleAudio()
  -- toggle audio enabled/disabled
  scheduleEvent(function()
    if modules.client_options then
      modules.client_options.setOption('enableAudio', not g_settings.getBoolean('enableAudio'))
    end
  end, 50)
end
...
function EnterGame.doLogin(account, password, token, host)
  ...
  G.authenticatorToken = token or "" -- Token field removed from UI
  G.stayLogged = true
